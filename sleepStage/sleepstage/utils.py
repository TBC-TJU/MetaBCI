...
# 我改动了
def get_balance_class_oversample(x, z, y):
    ...
    for i in range(0,len(x)):
        ...
        z_ = z[i]
        y_ = y[i]
        class_labels = np.unique(y_)
        n_max_classes = -1
        for c in class_labels:
            n_samples = len(np.where(y_ == c)[0])
        ...
        for c in class_labels:
            ...
            tmp_x = np.repeat(x_[idx], n_repeats, axis=0)
            tmp_z = np.repeat(z_[idx], n_repeats, axis=0)
            tmp_y = np.repeat(y_[idx], n_repeats, axis=0)
            n_remains = n_max_classes - len(tmp_x)
            if n_remains > 0:
                sub_idx = np.random.permutation(idx)[:n_remains]
                tmp_x = np.vstack([tmp_x, x_[sub_idx]])
                tmp_z = np.vstack([tmp_z, z_[sub_idx]])
                tmp_y = np.hstack([tmp_y, y_[sub_idx]])
        ...
        data_x.append(balance_x)
        data_z.append(balance_z)
        data_y.append(balance_y)
