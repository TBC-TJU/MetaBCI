...
class TinySleepNet(object):
    ...
    def __init__(
        ...
        if self.config['training_mode'] == 'finetune':
            fine_tune = True
            self.finetune_model_dir = os.path.join(self.finetune_model_dir, "best_ckpt")
        # 要恢复的层
        restore_layers = ['EEGcnn', 'EOGcnn', 'num_blocks_0', 'num_blocks_1', 'ln', 'ResidualBlock_dense_1',
                          'ResidualBlock_dense_2', 'lastconv', 'rnn']
        # 要冻结的层
        frozen_layers = ['EEGcnn', 'EOGcnn', 'num_blocks_0', 'num_blocks_1', 'ln', 'ResidualBlock_dense_1',
                         'ResidualBlock_dense_2', 'lastconv']
        ...
        # print(111,self.EEGsignals.shape)
        ...
        #         # ResidualBlock 残差
        se = self.ln(se)
        last_dim = se.shape[-1].value
        sub = layers.Dense(last_dim, activation='relu', name="ResidualBlock_dense_1")(se)
        sub = tf.layers.dropout(sub, rate=0.1, training=self.is_training)
        sub = layers.Dense(last_dim, activation='relu', name="ResidualBlock_dense_2")(sub)
        ...
        # rb += rb
        reshape_rb = tf.reshape(rb, [-1, 1, 1, last_dim])
        # conv
        reshape = layers.Conv2D(last_dim, (1, 1), activation='tanh',
                                padding='same', name="lastconv")(reshape_rb)
        # print("shangxia",reshape.shape)

        # 残差网络
        # print(reshape.shape)
        # reshape = self.build_resblock(last_dim, 1)(reshape)
        # reshape = tf.layers.dropout(reshape, rate=0.1, training=self.is_training)
        # print("asd", np.shape(layer1))
        # layer2 = self.build_resblock(last_dim, 64, stride=2)(layer1)
        # layer3 = self.build_resblock(last_dim, 32, stride=2)(layer2)
        # reshape = self.build_resblock(last_dim, 1, stride=2)(reshape)
        # reshape = tf.layers.dropout(rb, rate=0.1, training=self.is_training)
        ...
        if not testing:
            ...
            frozen_vars, restore_vars = self.finetune_vars(frozen_layers, restore_layers)

            var_list = tf.trainable_variables()

            # 去除冻结层的参数
            if fine_tune:
                var_list = [var for var in tf.trainable_variables() if var not in frozen_vars]
        ...
        # 从上次结束的地方继续训练。注意，这里的model_dir不能和输入目录一样
        if self.config['training_mode'] == 'continue_train':
            model_dir = ''
            self.continue_train_vars(model_dir)
            print(self.config['training_mode'], "111111111111111")
            is_restore = True
    ...
    def build_Sleepcnn(self):
        ...
        with tf.variable_scope("Sleepcnn") as scope:
            net = nn.conv1d("Sleepconv1d_1", self.SleepStages, 128, first_filter_size, first_filter_stride)
    ...
    def build_EOGcnn(self):
        ...
        first_filter_stride = int(self.config["sampling_rate"] / 32.0)
        ...
        with tf.variable_scope("EOGcnn") as scope:
            ...
            net = nn.max_pool1d("EOGmaxpool1d_2", net, 8, 8)
    ...
    def multihead_attention(self, queries, keys, values,
        ...
        with tf.variable_scope(scope, reuse=tf.AUTO_REUSE):
            ...
            # Attention
            scaled_attention, attention_weights = self.scaled_dot_product_attention(Q_split, K_split, V_split,
    ...
    def train(self, minibatches):
        ...
        if not self.use_rnn:
            for x, y, z, s, w, sl, re in minibatches:
                feed_dict = {
                    self.EEGsignals: x,
                    self.EOGsignals: y,
    ...
    def finetune_vars(self, frozen_layers, restore_layers):
        trainable_vars = tf.trainable_variables()
        trainable_vars_dict = {var.name: var for var in trainable_vars}

        trainable_vars_scopes = [var_name.split('/')[0] for var_name in trainable_vars_dict.keys()]
        layers = set(trainable_vars_scopes)

        print("Model's layers:", layers)
        ...
        return frozen_vars, restore_vars

    def continue_train_vars(self, model_dir):
        best_ckpt_path = os.path.join(model_dir, "best_ckpt")
        if os.path.exists(best_ckpt_path):
            if os.path.isfile(os.path.join(best_ckpt_path, "checkpoint")):
                saver_restore = tf.train.Saver(tf.global_variables())
                # Restore the last checkpoint
                latest_checkpoint = tf.train.latest_checkpoint(best_ckpt_path)
                saver_restore.restore(self.sess, latest_checkpoint)
                graph = tf.get_default_graph()
                logger.info("Best model for fine tuning restored from {}".format(latest_checkpoint))
