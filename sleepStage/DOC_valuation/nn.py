...
# def lrelu(inputs, name,leak=0.1):
#     with tf.variable_scope(name) as scope:
#         return tf.maximum(inputs, leak * inputs, name=scope)


def lrelu(inputs, name, leak=0.01):
    with tf.variable_scope(name) as scope:
        f1 = 0.5 * (1 + leak)
        f2 = 0.5 * (1 - leak)
...
def conv1d(
    ...
    with tf.variable_scope(name) as scope:
        outputs = tf.layers.conv2d(
            inputs=inputs,
            filters=n_filters,
            kernel_size=(filter_size, filter_size2),#修改
            strides=(stride_size, stride_size2),#修改
            padding=padding,
            use_bias=use_bias,
            kernel_initializer=weight_initializer,
            bias_initializer=bias_initializer,
...
def max_pool1d(
    ...
    with tf.variable_scope(name) as scope:
        outputs = tf.layers.max_pooling2d(
            inputs,
            pool_size=(pool_size, pool_size2),
            strides=(stride_size, stride_size2),
            padding=padding
        )
...
def adam_optimizer_clip(
    ...
    with tf.variable_scope("adam_optimizer") as scope:
        ...
        print("asdasdasdasd", np.shape(global_step))
        return apply_gradient_op, grads_and_vars_op
...
#sinc卷积层
def sinc(x):
    return tf.where(tf.abs(x) < 1e-8, tf.ones_like(x), tf.sin(np.pi * x) / (np.pi * x))


def sinc_conv1d(inputs, num_filters, kernel_size, sample_rate, stride_size, padding="SAME", bias=None):
    batch_size, width, height, channels = inputs.get_shape().as_list()
    timesteps = tf.range(-(kernel_size // 2), kernel_size // 2 + 1, dtype=tf.float32)
    timesteps = tf.reshape(timesteps, [1, -1, 1, 1])  # Reshape for broadcasting
    cutoff_frequencies = (sample_rate * 0.5) * np.linspace(0.1, 0.9, num_filters)  # Adjust this range as needed
    cutoff_frequencies = tf.constant(cutoff_frequencies, dtype=tf.float32)
    cutoff_frequencies = tf.reshape(cutoff_frequencies, [-1, 1, 1, 1])  # Reshape for broadcasting

    sinc_filters = 2 * cutoff_frequencies * sinc(2 * np.pi * cutoff_frequencies * timesteps / sample_rate)

    sinc_filters = tf.transpose(sinc_filters, perm=[1, 0, 2, 3])
    sinc_filters = tf.tile(sinc_filters, [1, 1, height, channels])

    outputs = tf.nn.conv2d(inputs, sinc_filters, strides=[1, stride_size, 1, 1], padding=padding)
