...
def compute_performance(cm):
    ...
    return total, n_each_class, acc, mf1, precision, recall, f1, ka
...
def predict(
    ...
    for fold_idx in range(config["n_folds"]):
        ...
        s_cm = skmetrics.confusion_matrix(y_true=s_trues, y_pred=s_preds, labels=[0,1])
    ...
    logger.info("Per-class Kappa-Score: " + " ".join(["{:.1f}".format(metrics[7] * 100.0) ]))
