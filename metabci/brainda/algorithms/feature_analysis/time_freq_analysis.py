# -*- coding: utf-8 -*-
"""
Time-frequency analysis, including continuous wavelet transform, short-time Fourier transform, and Hilbert transform.
"""
...
def spectplot(times, freqs, feature, cmap='viridis', vmin=None, vmax=None,
              xlabel='Time(s)', ylabel='Frequency(Hz)', text_label='Power (\\muV^2/Hz)',
              fontsize_labels=10, fontweight_labels='bold', fontcolor_labels='black', fontstyle_labels='normal',
              fontsize_ticks=10, fontweight_ticks='bold', fontcolor_ticks='black', fontstyle_ticks='normal',
              fontsize_title=14, fontweight_title='bold', fontcolor_title='limegreen', fontstyle_title='oblique',
              title_label="Spectral Analysis", distance=0.3,
              fontsize_bar=10, fontweight_bar='bold', fontcolor_bar='black', fontstyle_bar='normal'):
    """ Spectral analysis.
    ...
    distance : float
        The horizontal distance between the time-frequency graph and the title of colorbar.
    ...
    plt.title(title_label, fontsize=fontsize_title, fontweight=fontweight_title,
              color=fontcolor_title, fontstyle=fontstyle_title)
    plt.text(np.max(times) + distance, np.max(freqs) / 2, text_label,
             fontsize=fontsize_bar, fontweight=fontweight_bar, color=fontcolor_bar, fontstyle=fontstyle_bar,
             rotation=90, verticalalignment='center', horizontalalignment='center')
    plt.colorbar()


def _htplot(times, sfreq, signal, envelope, frequency, phase, title="Hilbert Transform",
            ...
            phase_label='instantaneous phase', frequency_label='instantaneous frequency', axis_label='Time(s)',
            signal_color='blue', envelope_color='orange', frequency_color='red', phase_color='green',
            fontsize=10, fontweight='bold', fontcolor='black', fontstyle='normal', linewidth=1, linestyle='solid',
            bbox_to_anchor=(0.88, 0.33), labelcolor='black', framealpha=0.8, facecolor='inherit', edgecolor='0.8',
    ...
    signal_label、envelope_label、phase_label、frequency_label、axis_label : str
        The label of the orignal signal (amplitude envelope, instant frequency, instant phase、axis).
    signal_color、envelope_color、phase_color、frequency_color、axis_color : str
        The line color of the orignal signal (amplitude envelope, instant frequency, instant phase).
    fontsize : float or str
        If the font size is str, supported values are 'xx-small', 'x-small', 'small', 'medium', 'large',
        'x-large', 'xx-large'.
    fontweight : numeric value in range 0-1000 or str
    ...
    linewidth : float
        The line width.
    linestyle : str
        The line style, by default 'solid', supported values are 'dashed', 'dash-dot', 'dotted'.
...
class CWT(BaseEstimator, TransformerMixin):
    ...
    We apply CWT after data pre-processing in the version, to perform time-frequency analysis of the
    EEG signal. Performs a continuous wavelet transform on data, using the wavelet function. A CWT
    performs a convolution with data using the wavelet function, which is characterized by a width
    parameter and length parameter. The wavelet function is allowed to be complex.
    ...
    n_samples : int
        ...
        When the default is None, it is equal to the total number of sample points per trial.
    dtype : data-type, optional
        The desired data type of output. Defaults to float64 if the output of wavelet is real
    ...
    cmap : str or colormap
        ...
        by default 'viridis', supported values are 'summer', 'GnBu_r', 'RdPu_r', 'YlGn_r'......
    vmin, vmax : float, optional
        When using scalar data and no explicit norm, vmin and vmax define the data range that the colormap covers.
        By default 'None', the colormap covers the complete value range of the supplied data.
    title : str
        The title of the time-frequency graph.
    distance : float
        The horizontal distance between the time-frequency graph and the title of colorbar.
    ...
    References
    ----------
    .. [1] https://docs.scipy.org/doc/scipy/reference/generated/scipy.signal.cwt.html#scipy.signal.cwt.
    .. [2] https://github.com/scipy/scipy/blob/v1.10.1/scipy/signal/_wavelets.py#L405-L492.
    ...
    .. code-block:: python
        ...
        from metabci.brainda.algorithms.feature_analysis.time_frequency_analysis import CWT
        cwt = CWT(n_jobs=None, fs=srate, wavelet=signal.morlet2, freqs=freq, omega0=5,
                ...
                fontsize_title=12, fontweight_title='bold',
                fontcolor_title='black', fontstyle_title='oblique',
                title="Class0: Complex Morlet Wavelet Transform of 'C3'", distance=0.1)
        cwt.fit(X_class0)
    ...
    Note
    ----
    Font format (fontsize、fontweight、fontcolor、fontstyle...) can be set according to your needs.


    """

    def __init__(self, n_jobs=None, fs=None, wavelet=signal.morlet2, freqs=None, omega0=5, n_samples=None,
                 dtype="complex128", trail_id=1, channel_id=1, cmap='viridis', vmin=None, vmax=None,
                 fontsize_title=14, fontweight_title='bold', fontcolor_title='limegreen', fontstyle_title='oblique',
                 title="Complex Morlet Wavelet Transform", distance=0.05):
    ...
    def transform(self, X: ndarray):
        ...
        for trial in range(n_trails):
            for channel in range(self.n_channels):
                self.CWT_complexmatrix[trial, channel, :, :] = signal.cwt(X[trial, channel, :self.n_samples],
                                                                          self.wavelet, widths, self.dtype)
    ...
    def draw(self):
        ...
        spectplot(t_index, self.freqs,
                  self.CWT_spectrum_energy[self.trail_id, self.channel_id, :, :],
...
class STFT(BaseEstimator, TransformerMixin):
    ...
    We apply STFT after data pre-processing in the version, to perform time-frequency analysis of the EEG signal.
    ...
    window : str or tuple or array_like, optional
        Desired window to use. If `window` is a string or tuple, it is passed to `get_window` to generate the
        window values, which are DFT-even by default. See `get_window` for a list of windows and required parameters.
        If `window` is array_like it will be used directly as the window and its length must be nperseg. Defaults
        to a Hann window.
    n_samples : int
        Maximum threshold of time window for time-frequency analysis.
        When the default is None, it is equal to the total number of sample points per trial.
    ...
    boundary : str or None, optional
        Specifies whether the input signal is extended at both ends, and how to generate the new values, in order to
        center the first windowed segment on the first input point. This has the benefit of enabling reconstruction of
        the first input point when the employed window function starts at zero. Valid options are ``['even', 'odd',
        'constant', 'zeros', None]``. Defaults to 'zeros', for zero padding extension. I.e. ``[1, 2, 3, 4]`` is
        extended to ``[0, 1, 2, 3, 4, 0]`` for ``nperseg=3``.
    padded : bool, optional
        Specifies whether the input signal is zero-padded at the end to make the signal fit exactly into an integer
        number of window segments, so that all of the signal is included in the output. Defaults to `True`.
    ...
    cmap : str or colormap
        ...
        by default 'viridis', supported values are 'summer', 'GnBu_r', 'RdPu_r', 'YlGn_r'......
    vmin, vmax : float, optional
        When using scalar data and no explicit norm, vmin and vmax define the data range that the colormap
        covers. By default 'None', the colormap covers the complete value range of the supplied data.
    title : str
        The title of the time-frequency graph.
    distance : float
        The horizontal distance between the time-frequency graph and the title of colorbar.
    ...
    .. code-block:: python
        ...
        from metabci.brainda.algorithms.feature_analysis.time_frequency_analysis import STFT
        stft = STFT(n_jobs=None, fs=srate, nperseg=64, nfft=X.shape[2],
                trail_id=20, channel_id=14, cmap='GnBu_r', vmin=None, vmax=None,
                fontsize_title=12, fontweight_title='bold',
                fontcolor_title='black', fontstyle_title='oblique',
                title="Trial 20-Channel 14: Short Time Fourier Transform", distance=0.25)
        stft.fit(X)
    ...
    def __init__(self, n_jobs=None, fs=None, window='hann', n_samples=None, nperseg=None,
                 noverlap=None, nfft=None, detrend=False, return_onesided=True,
                 boundary='zeros', padded=True, axis=-1,
                 trail_id=1, channel_id=1, cmap='viridis', vmin=None, vmax=None,
                 fontsize_title=14, fontweight_title='bold',
                 fontcolor_title='limegreen', fontstyle_title='oblique',
                 title="Short Time Fourier Transform", distance=0.05):
        self.n_jobs = n_jobs
        self.fs = fs
        self.window = window
        self.n_samples = n_samples
    ...
    def transform(self, X: ndarray):
        ...
        self.data = signal.stft(X[0, 0, :self.n_samples], self.fs,
                                self.window, self.nperseg, self.noverlap, self.nfft,
                                self.detrend, self.return_onesided, self.boundary,
                                self.padded, self.axis)
        # data is a tuple, data[0]=f, data[1]=t, data[2]=Zxx(needed complex
        # matrix)
        self.STFT_complexmatrix = np.zeros((n_trails, self.n_channels,
                                            self.data[2].shape[0],
                                            self.data[2].shape[1]),
                                            dtype="complex128")
        for trial in range(n_trails):
            for channel in range(self.n_channels):
                self.data = []
                self.data = signal.stft(X[trial, channel, :self.n_samples], self.fs,
                                        self.window, self.nperseg, self.noverlap, self.nfft,
                                        self.detrend, self.return_onesided, self.boundary,
                                        self.padded, self.axis)
    ...
    def draw(self):
        spectplot(self.data[1], self.data[0],
                  self.STFT_spectrum_energy[self.trail_id, self.channel_id, :, :],
                  cmap=self.cmap, vmin=self.vmin, vmax=self.vmax,
                  title_label=self.title, distance=self.distance,
                  fontsize_title=self.fontsize_title,
                  fontweight_title=self.fontweight_title,
...
class HT(BaseEstimator, TransformerMixin):
    ...
    We apply HT after data pre-processing in the version, to analyze the analytic signal, amplitude envelope,
    ...
    .. code-block:: python
        ...
        from metabci.brainda.algorithms.feature_analysis.time_frequency_analysis import HT
        ht = HT(n_jobs=None, fs=srate, axis=-1,
                trail_id=20, channel_id=14, title="Trial 20-Channel 14: Hilbert Transform",
                fontsize_title=14, fontweight_title='bold',
                fontcolor_title='black', fontstyle_title='oblique',
                ...
                bbox_to_anchor=(0.99,0.38), labelcolor='black', framealpha=0.7,
                facecolor='white', edgecolor='0.8',
                labelspacing=0.5, handlelength=2, handleheight=0.7)
        ht.fit(X)
        HT_complexmatrix, amplitude_envelope, instant_phase, instant_frequency = ht.transform(X)
        ht.draw()
    ...
    def __init__(self, n_jobs=None, fs=None, N=None, axis=-1, n_samples=None,
                 ...
                 fontsize_title=12, fontweight_title='bold', fontcolor_title='black', fontstyle_title='oblique',
                 signal_label='orignal signal', envelope_label='amplitude envelope',
                 phase_label='instantaneous phase', frequency_label='instantaneous frequency', axis_label='Time(s)',
                 signal_color='blue', envelope_color='orange', frequency_color='red', phase_color='green',
                 fontsize=10, fontweight='bold', fontcolor='black', fontstyle='normal',
    ...
    def transform(self, X: ndarray):
        n_trails = X.shape[0]
        self.HT_complexmatrix = np.zeros((n_trails, self.n_channels, self.n_samples),
                                         dtype="complex128")
        ...
        for trial in range(n_trails):
            for channel in range(self.n_channels):
                ...
                self.instant_frequency[trial, channel, :] = (np.diff(np.unwrap(self.instant_phase[trial, channel, :]))
                                                             / (2.0 * np.pi) * self.fs)
        return self.HT_complexmatrix, self.amplitude_envelope, self.instant_phase, self.instant_frequency

    def draw(self):
        t = np.arange(self.n_samples) / self.fs
        _htplot(t, self.fs, np.real(self.HT_complexmatrix[self.trail_id, self.channel_id, :]),
                ...
                self.title, self.fontsize_title, self.fontweight_title, self.fontcolor_title, self.fontstyle_title,
                self.signal_label, self.envelope_label, self.phase_label, self.frequency_label, self.axis_label,
                self.signal_color, self.envelope_color, self.frequency_color, self.phase_color,
                self.fontsize, self.fontweight, self.fontcolor, self.fontstyle,
                self.linewidth, self.linestyle,
                self.bbox_to_anchor, self.labelcolor, self.framealpha,
                self.facecolor, self.edgecolor,
                self.labelspacing, self.handlelength, self.handleheight)
        return
