...
class Net(nn.Module):
    ...
    def __init__(self, datalen, classnum=10, Nb=3, Nk=3):
...
class DisComNet(BaseEstimator, TransformerMixin, ClassifierMixin):
    ...
    Phi : ndarray
        The matrix obtained by global template alignment (GTA) calculated individually in each band, shape(n_bands, n_channels, n_channels).
    classes_ : ndarray
    ...
    def fit(self, X, y=None) -> 'DisComNet':
        ...
        self.W = np.zeros((X.shape[1], self.Nk, X.shape[2]))
        self.Phi = np.zeros((X.shape[1], X.shape[2], X.shape[2]))
        traindata = np.zeros((X.shape[0], X.shape[1], self.Nk, 2 * X.shape[3]))
        ...
        ran = np.random.randint(int(X.shape[0] / nEvents), size=(1,))[0]
    ...
    def predict(self, X):
        ...
        for iBand in range(X.shape[1]):
            for idata in range(X.shape[0]):
                testdata[idata, iBand, :, :] = self.W[iBand, :, :] @ np.concatenate(
                    (X[idata, iBand, :, :], self.Phi[iBand, :, :] @ X[idata, iBand, :, :]), axis=-1)
    ...
    def get_filter(self, X, y):
        ...
        for i, ilabel in enumerate(label):
            ...
            for jtrial in range(len(label_idx)):
                Sw += (X[label_idx[jtrial], :, :] - templates[i, :, :]) @ (
                        X[label_idx[jtrial], :, :] - templates[i, :, :]).T
        Sb /= len(label)
        Sw /= nTrials
    ...
    def fit_deep_model(self, datas, validation_X, validation_y):
        ...
        model = Net(2 * self.datalen, classnum=self.classes).to(self.device)
        mini_loss = 10
