...
"""
task-related correlation analysis network (TRCA-Net). 
A neural network-based SSVEP decoding method termed task-related correlation analysis network (TRCA-Net) [1]_
...
class TRCANet(BaseEstimator, TransformerMixin, ClassifierMixin):
    ...
    def fit(self, X: ndarray, y: ndarray) -> 'TRCANet':
        ...
        """
        self.classes_ = np.unique(y)
        traindata = np.zeros((X.shape[0], X.shape[1], len(self.classes_), X.shape[3]))
        for iband in range(X.shape[1]):
            self.trca_estimators.append(TRCA2(n_components=1, ensemble=True).fit(X[:, iband, :, :], y))
            traindata[:, iband, :, :] = self.trca_estimators[iband].transform(X[:, iband, :, :])
        ...
        # num_bands=1, num_channels=9, num_classes=10, nsamples=150
        model = DNN(num_bands=X.shape[1], num_channels=X.shape[-2], nsamples=X.shape[-1],
                    num_classes=len(self.classes_)).to(self.device)
        # train the model
    ...
    def predict(self, X: ndarray):
        ...
        """
        testdata = np.zeros((X.shape[0], X.shape[1], len(self.classes_), X.shape[3]))
        for iband in range(X.shape[1]):
            testdata[:, iband, :, :] = self.trca_estimators[iband].transform(X[:, iband, :, :])
