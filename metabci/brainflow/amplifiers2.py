...
class Marker(RingBuffer):
    ...
    def get_current_label(self) -> Optional[int]:
        """Get the current event label if available"""
        # print('receive label:',self.current_label)
        ...
        for work_name in self._workers:
            ...
            for sample in samples:
                ...
                if marker(sample[-1]) and worker.is_alive():
                    data = marker.get_epoch()
                    label = marker.get_current_label()
                    worker.put((data, label))
...
class Neuracle(BaseAmplifier):
    ...
    def __init__(self,
        ...
        # Initialize DataServerThread from neuracle_api
        self.data_server = DataServerThread(sample_rate=srate, t_buffer=60)
        self.data_server.connect(hostname=device_address[0], 
                                 port=device_address[1])

        # Wait until ready
        while not self.data_server.isReady():
            time.sleep(0.01)

        # Initialize buffer for storing data chunks
        self.buffer = []
        self.last_update_time = time.time()
        self.data_lock = threading.Lock()

    def recv(self):
        """Receive and process data chunks from DataServerThread."""
        samples = []

        # Get updated data from DataServerThread
        with self.data_lock:
            # Get new data points
            update_count = self.data_server.GetDataLenCount()
            if update_count > 0:
                # Retrieve new data (shape: [channels, points])
                new_data = self.data_server.buffer.getUpdate()

                # Reset update counter
                self.data_server.ResetDataLenCount()

                # Transpose to [points, channels]
                new_data = new_data.T

                # Append to samples list
                samples = new_data.tolist()

                # Reset trigger channel in buffer
                self.data_server.ResetTriggerChanofBuff()
                # print(samples)
