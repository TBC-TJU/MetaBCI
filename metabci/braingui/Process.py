...
class Preprocess_Form(QWidget):
    ...
    def downsample_data(self, dialog_class):
        self.factor = int(dialog_class.lineEdit_factor.text())
    ...
    def cut_data(self, dialogclass):
        ...
        if dialogclass.checkBox_2.isChecked():
            print(dialogclass.checkBox_2.isChecked())
            mark_list.append(2)
        ...
        self.Function.cut_data(pathname=self.folder_path, time=time_list, mark_list=mark_list, fs=self.fs, loaddata=self.load_data, factor=self.factor)
