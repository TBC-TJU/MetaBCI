...
from metabci.brainda.algorithms.deep_learning.trca_net import TRCANet
...
for k in range(n_splits):
    ...
    trcanet = TRCANet(nclass=40)
    accs.append(trcanet.fit(X_train, y_train).score(X[test_ind], y[test_ind]))
    print(accs)

print('TRCANet:  ', np.mean(accs))
