...
def raw_hook(raw, caches):
    ...
    raw.filter(7, 40, l_trans_bandwidth=2,h_trans_bandwidth=5,
        phase='zero-double')
...
def train_model(X, y, srate=1000):
    ...
    # brainda.algorithms.decomposition.dsp.FBDSP
...
def time_feature(X, meta, dataset, event, channel, latency=0):
    ...
    data_mean = Feature_R.stacking_average(np.squeeze(Feature_R.data[:, Feature_R.chan_ID, :]), _axis=0)
    ax = plt.subplot(2, 1, 1)
    sample_num = int(Feature_R.fs*Feature_R.data_length)
    # 画出模板信号及其振幅调用TimeAnalysis.plot_single_trial()
    loc, amp, ax = Feature_R.plot_single_trial(data_mean, sample_num=sample_num, axes=ax, amp_mark='peak', time_start=0,
                                               time_end=sample_num-1)
    plt.title("(a)", x=0.03, y=0.86)
    # 画出多试次信号调用TimeAnalysis.plot_multi_trials()
    ax = plt.subplot(2, 1, 2)
    ax = Feature_R.plot_multi_trials(np.squeeze(Feature_R.data[:, Feature_R.chan_ID, :]), sample_num=sample_num, axes=ax)
    ...
    Feature_R.plot_topomap(data_map, loc, fig=fig2, channels=Feature_R.All_channel, axes=ax)
    plt.show()
...
def frequency_feature(X,chan_names,event,SNRchannels,plot_ch,srate=1000):
    ...
    plt.plot(f,den)
    plt.text(8, den[f==8][0], '{:.2f}'.format(den[f==8][0]), fontsize=20)
    plt.text(16, den[f==16][0], '{:.2f}'.format(den[f==16][0]), fontsize=20)
    plt.text(24, den[f==24][0], '{:.2f}'.format(den[f==24][0]), fontsize=20)
    plt.title('OZ FFT')
    plt.xlim([0, 30])
    plt.xlabel('fre [Hz]')
    plt.ylabel('PSD [V**2]')
    ...
    for i in range(mean_data.shape[0]):
        # 调用FrequencyAnalysis.power_spectrum_periodogram()
        f, den = Feature_R.power_spectrum_periodogram(mean_data[i])
        y_list.append(den[f==8][0])
    y_list = np.array(y_list)
    y_list = y_list[:, None]
    # 8Hz处幅值脑地形图,调用FrequencyAnalysis.plot_topomap()
    Feature_R.plot_topomap(y_list, chan_names)
    # 信噪比,调用FrequencyAnalysis.signal_noise_ratio()
    SNR = Feature_R.signal_noise_ratio(data=mean_data, srate=srate, T=2, channel=SNRchannels)
...
def time_frequency_feature(X,y,chan_names,srate=1000):
    ...
    plt.text(t_lim[1] + 0.04, f_lim[1] / 2, 'Power (\muV^2/Hz)', rotation=90, verticalalignment='center',
    ...
    plt.plot(time, realEnv, "k", marker='o', markerfacecolor='white', label=u"real part")
...
if __name__ == '__main__':
    # 初始化参数
    srate = 1000                                                            # 放大器的采样率
    stim_interval = [(0.14, 2.14)]                                          # 截取数据的时间段
    ...
    dataset = MetaBCIData(
        ...
        paradigm='ssvep', pattern='ssvep')                               # declare the dataset
    paradigm = SSVEP(
    ...
    # 时域分析
    time_feature(X, meta, dataset, '1', ['Oz'])
    # 频域分析
    frequency_feature(X,pick_chs,'1','Oz',-2,srate)
    # 时频域分析
    time_frequency_feature(X, y,pick_chs)
