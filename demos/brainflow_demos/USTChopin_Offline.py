...
def get_rawdata(experiment_data):
    ...
    for data in experiment_data:
        ...
        elif data['marker1']== 0: #if raw[list_eeg,:].shape[1]==800:
            EEG_All.append(raw[list_eeg,-2000:])
            # EEG_All.append(signal.filtfilt(b11, a11, raw[list_eeg,:],axis=1))
            EMG_All.append(np.abs(signal.filtfilt(b12, a12,raw[list_emg,-2000:],axis=1)))
...
def prepare_dataset(length_eeg,length_emg,step,EEG_All,EMG_All,Label_1_All,Label_2_All):
    train_eeg,train_label_1,train_label_2 = segmentation(EEG_All[0:10*3],Label_1_All[0:10*3],Label_2_All[0:10*3],length_eeg,length_eeg,step)#前4轮数据做训练数据
    train_emg,train_label_1,train_label_2 = segmentation(EMG_All[0:10*3],Label_1_All[0:10*3],Label_2_All[0:10*3],length_eeg,length_emg,step)

    test_eeg,test_label_1,test_label_2 = segmentation(EEG_All[10*3:],Label_1_All[10*3:],Label_2_All[10*3:],length_eeg,length_eeg,step)
    test_emg,test_label_1,test_label_2 = segmentation(EMG_All[10*3:],Label_1_All[10*3:],Label_2_All[10*3:],length_eeg,length_emg,step)
...
if __name__ == '__main__':
    ...
    experiment_data = np.load("ShuK_0821_2_data.npy",allow_pickle=True)
    ...
    length_eeg,length_emg,step = 200,200,100
    ...
    if model_type == 'LDA' or model_type == 'SVM' or model_type == 'RandomForest':
        ...
        dump(Trained_model, './'+Exp_date+'_{}_Gmodel.joblib'.format(model_type))
        dump(Trained_model_2, './'+Exp_date+'_{}_Gmodel_attention.joblib'.format(model_type))
