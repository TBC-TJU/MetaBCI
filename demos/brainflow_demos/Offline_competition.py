...
from metabci.brainda.algorithms.deep_learning.trca_net import myTRCA, TRCANet
...
def train_model(X, y, srate=1000):
    ...
    model = TRCANet(nclass=8)
    model.fit(X, y)
    # filterweights = [(idx_filter + 1) ** (-1.25) + 0.25 for idx_filter in range(3)]
    # wp = [(20, 90), (40, 90), (60, 90)]
    # ws = [(18, 92), (38, 92), (58, 92)]
    # filterbank = generate_filterbank(wp, ws, srate=250, order=15, rp=0.5)
    # freq_list = np.arange(21, 29, 1)
    # Yf = generate_cca_references(freq_list, srate=250, T=0.5, n_harmonics=3)
    # estimator = FBTDCA(filterbank, padding_len=5, n_components=8,filterweights=np.array(filterweights))
    # model = estimator.fit(X=X, y=y, Yf=Yf)
    return model
...
def model_predict(X, srate=1000, model=None):
    ...
    X = X / np.std(X, axis=(-1, -2), keepdims=True)
    # FBDSP.predict()预测标签
    p_labels, possi = model.predict(X)
    return p_labels, possi
...
def offline_validation(X, y, srate=1000):
    ...
    for train_ind, test_ind in spliter.split(X, y=y):
        ...
        # 此处使用SAME扩增
        same = SAME(fs=1000, Nh=2, flist=freq_list, n_Aug=10)
        ...
        model = train_model(X_train_new, y_train_new, srate=srate)          # 训练模型
        p_labels,_ = model_predict(X_test, srate=srate, model=model)  # 预测标签
        kfold_accs.append(np.mean(p_labels == y_test))                # 记录正确率
        print(kfold_accs)
    print(len(kfold_accs))
...
if __name__ == '__main__':
    ...
    # 存一下模型
    freq_list = np.arange(21, 29, 1)
    same = SAME(fs=1000, Nh=3, flist=freq_list, n_Aug=7)
    same.fit(X, y)
    X_aug, y_aug = same.augment()
