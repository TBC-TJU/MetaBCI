...
import numpy as np
from metabci.brainstim.paradigm_ssvep import (
    SSVEP,
    # P300,
    # MI,
    # AVEP,
...
if __name__ == "__main__":
    ...
    SSVEP
    """
    n_elements, rows, columns = 8, 1, 8  # n_elements 指令数量;  rows 行;  columns 列
    stim_length, stim_width = 180, 600  #
    stim_color, tex_color = [1, 1, 1], [1, 1, 1]  # 指令的颜色，文字的颜色
    fps = 240  # 屏幕刷新率
    stim_time = 0.6  # 刺激时长
    stim_opacities = 1  # 刺激对比度
    freqs = np.arange(21, 29, 1)  # 指令的频率
    ...
    basic_ssvep.black_block()
    basic_ssvep.white_block()
    basic_ssvep.grey_block()
    basic_ssvep.config_response()

    bg_color = np.array([-1, -1, -1])  # 背景颜色(0.3,0.3,0.3)
    display_time = 1  # 范式开始1s的warm时长
    index_time = 1  # 提示时长，转移视线
    rest_time = 0.2  # 提示后的休息时长
    response_time = 0.5  # 在线反馈
    # port_addr = "COM8"  #  0xdefc                                  # 采集主机端口
    port_addr = 0X4EFC  #  0xdefc
    nrep = 4  # block数目
